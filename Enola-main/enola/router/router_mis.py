...
def main():
    # 1×3 “chip” (width=3, height=1)
    # chip_dim     = (3, 1)
    # n_qubits     = 3
    # # stage 0 has a single gate between qubit 0 and qubit 2
    # list_gates   = [[(0, 2)]]
    # # initial mapping: 0→(0,0), 1→(1,0), 2→(2,0)
    # mapping      = [(0,0), (1,0), (2,0)]
    # routing_str  = ""     # unused in this test
    # reverse_back = False  # no reverse pass
    # use_euclid   = True
    # use_window   = False  # unused

    # data, final_map, _ = route_qubit_parallel_neighbors(
    #     chip_dim, n_qubits, 0,
    #     list_gates, mapping,
    #     routing_str, reverse_back,
    #     use_euclid, use_window
    # )
    
# -----------------------------------------------------------------------------
# Test setup: 5 qubits in a row, 3 two‐qubit gates in one Rydberg stage
# -----------------------------------------------------------------------------
    # chip_dim    = (5, 1)                                    # 5 columns × 1 row
    # n_qubits    = 5
    # initial_map = [(i, 0) for i in range(n_qubits)]        # [(0,0),(1,0),(2,0),(3,0),(4,0)]
    # list_gates  = [
    #     [ (0, 4),  # qubit 0 ↔ 4, distance = 4
    #     (1, 3),  # qubit 1 ↔ 3, distance = 2
    #     (2, 3)   # qubit 2 ↔ 3, distance = 1 (already adjacent)
    #     ]
    # ]
    chip_dim = (3, 3)
    n_qubits = 9
    ...
    data, final_map, _ = route_qubit_mis(
        chip_dim,
        n_qubits,
        list_gates      = list_gates,
        qubit_mapping         = initial_map.copy(),
    ...
    # pretty-print the layers (ignore runtime)
    # data["runtime"] = 0.0
    # print(json.dumps(data, indent=2))
    # print("Final mapping:", final_map)
