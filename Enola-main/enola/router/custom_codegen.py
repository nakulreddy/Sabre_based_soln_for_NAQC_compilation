...
class SwapNeighbor(ComboInst):
    """
    Swap two adjacent qubits Q and N by:
      – using the full context of all active columns/rows,
      – then doing Activate/Move/Deactivate in the right order.
    """
    def __init__(self,
                 s: int,
                 col_objs: Sequence[Col],
                 ...
                 row_begin: Sequence[int],
                 row_end:   Sequence[int]):
        super().__init__('SwapNeighbor', prefix=f'SwapNeighbor_{q}_{n}', stage=s)

        # print("_________SwapNeighbor__________")
        # print(row_begin)
        # print(col_begin)
        # print(q)
        # print(n)
        # print("__________SwapNeighbor__________")
        
        
        x_q, y_q = qubit_objs[q].x, qubit_objs[q].y
        c_q, r_q = qubit_objs[q].c, qubit_objs[q].r
        
        x_n, y_n = qubit_objs[n].x, qubit_objs[n].y
        c_n, r_n = qubit_objs[n].c, qubit_objs[n].r
        
        original_n_x = qubit_objs[n].x
        original_q_x = qubit_objs[q].x

        original_n_y = qubit_objs[n].y
        original_q_y = qubit_objs[q].y

        # print("check for the row aod")
        # print(r_n)
        # print(r_q)
        # print("check for the col aod")
        # print(c_n)
        # print(c_q)

        # if (s != 1):
        #     # 1) ensure both traps are active
        if(r_q != r_n and c_q != c_n):
            self.insts.append(Activate(
                s,
                col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
                col_idx=col_idx,
                col_xs=[col_objs[i].x for i in col_idx],
                row_idx=row_idx,
                row_ys=[row_objs[i].y for i in row_idx],
                pickup_qs=[q, n],
                prefix=self.name + ':pickup_all'
            ))
        else:
            if(r_q == r_n):
                r_n = r_n + 1
                # if(r_n + 1 < len(row_objs)):
                #     r_n = r_n + 1
                # else:
                #     print("check for the row aod")
                #     print("is this it?")
            ...
            y_n = qubit_objs[n].y
            y_q = qubit_objs[q].y
            # print("in activating n")
            # print(qubit_objs[q].y)
            # print(y_q)
            ...
            x_q = qubit_objs[q].x
            # print("in activating n")
            # print(qubit_objs[q].x)
            # print(x_q)
            # row_objs[r_n].active = True
            # print(row_objs[r_n].active)
            self.insts.append(Activate(
                s,
                col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
                col_idx=[c_n],
                ...
                row_ys=[y_n],
                pickup_qs=[n],
                prefix=self.name + ':pickup_n'
            ))
            self.insts.append(Move(
                s=s,
                col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
                col_idx=[c_n],       col_begin=[x_n],     col_end=[x_n + AOD_SEP],
                row_idx=[r_n],       row_begin=[y_n],     row_end=[y_n + AOD_SEP],
                prefix=self.name + ':move_n_above'
            ))
            # print("before activate")
            # print(qubit_objs[q].x)
            # print(qubit_objs[q].y)
            # print(qubit_objs[q].c)
            # print(qubit_objs[q].r)
            # print(c_q)
            # print(r_q) 
            x_q = qubit_objs[q].x
            y_q = qubit_objs[q].y
            ...
            self.insts.append(Activate(
                ...
                row_ys=[y_q],
                pickup_qs=[q],
                prefix=self.name + ':pickup_q'
            ))

            self.insts.append(Move(
                s=s,
                col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
                col_idx=[c_q],       col_begin=[x_q],     col_end=[x_q - AOD_SEP],
                row_idx=[r_q],       row_begin=[y_q],     row_end=[y_q - AOD_SEP],
                prefix=self.name + ':move_q_below'
            ))
        

        # 2) move Q out to a “safe” X (using the full list if needed)
        direction_x = -1 if c_q < c_n else +1
        safe_x = x_q + (direction_x * X_SITE_SEP)

        # direction_y = -1 if r_q < r_n else +1
        # safe_y = y_q + (direction_y * RYD_SEP)

        # print("this is safe x")
        # print(col_objs[c_q].x)
        original_c_q_x = col_objs[c_q].x
        original_r_q_y = row_objs[r_q].y
        # print("lam")
        # print(original_r_q_y)
        # safe_x = x_q + RYD_SEP
        self.insts.append(Move(
            s=s,
            col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
            col_idx=[c_q],       col_begin=[x_q - AOD_SEP],     col_end=[safe_x],
            row_idx=[r_q],       row_begin=[y_q - AOD_SEP],     row_end=[y_q],
            prefix=self.name + f':move_q{q}_to_safe'
        ))
        
        # print("this is col_x")
        # print(x_q)
        # print(col_objs[c_n].x)

        # 3) move N into Q’s old spot
        move_n_to_q = Move(
            s=s,
            col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
            col_idx=[c_n],       col_begin=[x_n + AOD_SEP],     col_end=[x_q + AOD_SEP],
            row_idx=[r_n],       row_begin=[y_n + AOD_SEP],     row_end=[y_q + AOD_SEP],
            prefix=self.name + f':move_n{n}_into_q{q}'
        )
        self.insts.append(move_n_to_q)
        # print(move_n_to_q.emit_full())
        ...
        # print(r_n)
        # 4) drop off N
        # self.insts.append(Deactivate(
        #     s=s,
        #     col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
        #     col_idx=[c_n],       col_xs=[x_q],
        #     row_idx=[r_n],       row_ys=[y_q],
        #     dropoff_qs=[n],
        #     prefix=self.name + f':dropoff_n{n}'
        # ))

        
        # 4) drop off N into Q’s old spot
        # dropoff_col = c_q       # Q’s original column index
        # dropoff_row = r_q       # Q’s original row index
        # dropoff_x   = col_objs[dropoff_col].x
        # dropoff_y   = row_objs[dropoff_row].y
        ...
        #     row_idx=[dropoff_row],
        #     row_ys=[dropoff_y],
        #     dropoff_qs=[n],                    # you’re dropping off qubit N here
        #     prefix=self.name + f':dropoff_n{n}'
        # ))
        
        # for i in range(len(col_objs)):
        #     if (col_objs[i].active):
        #         print("this is col_objs")
        #         print(i)
        #         print(col_objs[i].x)
        ...
        dropoff_x   = col_objs[dropoff_col].x
        dropoff_y   = row_objs[dropoff_row].y
        # print("this is dropoff_x")
        # print(dropoff_x)

        trap_col = qubit_objs[n].c    # the AOD column index holding qubit N
        trap_row = qubit_objs[n].r    # the AOD row index holding qubit N
        ...
        # 5) move Q into N’s old spot
        self.insts.append(Move(
            s=s,
            col_objs=col_objs, row_objs=row_objs, qubit_objs=qubit_objs,
            col_idx=[c_q],       col_begin=[safe_x],  col_end=[x_n + AOD_SEP],
            row_idx=[r_q],       row_begin=[y_q],     row_end=[y_n + AOD_SEP],
            prefix=self.name + f':move_q{q}_into_n{n}'
        ))
        ...
        self.insts.append(Deactivate(
            ...
            row_ys=[y_n + AOD_SEP],
            dropoff_qs=[q],
            prefix=self.name + f':dropoff_q{q}'
        ))
        # print(f"swap on layer {s} is complete between {q} and {n}.")
...
class CodeGen():
    ...
    def builder(self, no_transfer: bool):
        ...
        init = self.builder_init(cols, rows, qubits, program)
        # print("HERE")
        # for i in range(len(self.layers)):
        #     print(self.layers[i])
        #     print("-------")
        for s in range(1, len(self.layers)):
            gates = self.layers[s]['gates']

            # 1) ALWAYS reload into AOD first
            #    (this makes col/row active for any subsequent Activate)
            if (not no_transfer) or s == 1:
                # print(self.layers[s]['qubits'][3])
                # print(self.layers[s]['qubits'][3])
                # print("these are the qubits...")
                # print(self.layers[s]['qubits'])
                # print(f"this is the layer: {s}" )
                # print(f"qubit 0 {qubits[0].x}, {qubits[0].y}, {qubits[0].c}, {qubits[0].r}")
                # print(f"qubit 1 {qubits[1].x}, {qubits[1].y}, {qubits[1].c}, {qubits[1].r}")
                # print(f"qubit 2 {qubits[2].x}, {qubits[2].y}, {qubits[2].c}, {qubits[2].r}")
                # print(f"qubit 3 {qubits[3].x}, {qubits[3].y}, {qubits[3].c}, {qubits[3].r}")
                # print(f"qubit 4 {qubits[4].x}, {qubits[4].y}, {qubits[4].c}, {qubits[4].r}")
                # print(f"qubit 5 {qubits[5].x}, {qubits[5].y}, {qubits[5].c}, {qubits[5].r}")
                # print(f"qubit 6 {qubits[6].x}, {qubits[6].y}, {qubits[6].c}, {qubits[6].r}")
                # print(f"qubit 7 {qubits[7].x}, {qubits[7].y}, {qubits[7].c}, {qubits[7].r}")
                # print(f"qubit 8 {qubits[8].x}, {qubits[8].y}, {qubits[8].c}, {qubits[8].r}")
                self.builder_reload(s, cols, rows, qubits, program)
                # pass
            
            # print(self.layers[s]['qubits'][3])
            # print(self.layers[s]['qubits'][2])

            # 2) If it's a pure‐swap layer, do your neighbor‐swap now
            # if gates and all(g.get("type") == "swap" for g in gates):
            print("swap layer ")
            print("-----")
            print(s)
            print("-----")
            other_cols_idx = [col.id for col in cols if col.active]
            aod_qubits = [q.id for q in qubits if q.array == 'AOD']
            if(aod_qubits != 2):
                print(aod_qubits)

            #     self.builder_swap_neighbors(s, cols, rows, qubits, program)
            #     continue   # skip move/offload/rydberg

            # 3) Otherwise do the normal flow
            
            other_cols_idx = [col.id for col in cols if col.active]
            # print(other_cols_idx)
            other_rows_idx = [row.id for row in rows if row.active]
            # print(other_rows_idx)
            self.builder_swap_neighbors(s, cols, rows, qubits, program)

            # print(program.emit_full())
            # print(f"here is the layer: {s}" )
            # print(f"qubit 0 {qubits[0].x}, {qubits[0].y}")
            # print(f"qubit 1 {qubits[1].x}, {qubits[1].y}")
            # print(f"qubit 2 {qubits[2].x}, {qubits[2].y}")
            # print(f"qubit 3 {qubits[3].x}, {qubits[3].y}")
            # print(f"qubit 4 {qubits[4].x}, {qubits[4].y}")
            # print(f"qubit 5 {qubits[5].x}, {qubits[5].y}")
            # print(f"qubit 6 {qubits[6].x}, {qubits[6].y}")
            # print(f"qubit 7 {qubits[7].x}, {qubits[7].y}")
            # print(f"qubit 8 {qubits[8].x}, {qubits[8].y}")
            # # self.builder_swap_neighbors(s, cols, rows, qubits, program)
            if not no_transfer:
                # print("this is s: ")
                # print(s)
    ...
    def builder_init(
        ...
        # # **Decide at layer 0** whether to do a swap‐only init or Rydberg init
        # gates0 = self.layers[0]['gates']
        
        # if gates0 and all(g.get("type") == "swap" for g in gates0):
        #     # if the very first layer is only swaps:
        #     self.builder_swap_neighbors(0, cols, rows, qubits, program)
        # else:
        #     # otherwise, use Rydberg initialization
    ...
    def builder_swap_neighbors(
        ...
        print("swap neighbors being called.")
        swap_obj = Swap(s)
        this_layer = self.layers[s]

        # look at every unique pair
        for q0 in range(self.n_q):
            for q1 in range(q0+1, self.n_q):
                # same row?
                # if qubits[q0].r != qubits[q1].r:
                #     continue
                print(f"before continue... {q1}")
                # are they both flagged AOD?
                if this_layer["qubits"][q0]["a"] != 1 or this_layer["qubits"][q1]["a"] != 1:
                    continue
                print(f"after continue... {q1}")
                # if this_layer["qubits"][q0]["a"] == 1 and this_layer["qubits"][q1]["a"] == 1:
                #     print(f"this is q0: {q0}")
                #     print(f"this is q1: {q1}")

                # neighbouring columns?
                c0 = this_layer["qubits"][q0]["c"]
                c1 = this_layer["qubits"][q1]["c"]

                r0 = this_layer["qubits"][q0]["r"]
                r1 = this_layer["qubits"][q1]["r"]
                # if abs(c0 - c1) != 1:
                #     print(abs(c0-c1))
                #     print(f"this is q0: {q0}")
                #     print(f"this is q1: {q1}")
                ...
                # decide left vs right by x‐coordinate
                left, right = (q0, q1)
                if qubits[q0].x > qubits[q1].x:
                    left = q1
                    right = q0
                elif qubits[q0].x == qubits[q1].x:
                    # if they are in the same column, decide by row
                    if qubits[q0].y < qubits[q1].y:
                        left = q0
                        right = q1
                    else:
                        left = q1
                        right = q0                   
                # left, right = (q0, q1) if qubits[q0].x < qubits[q1].x else (q1, q0)
                ...
                other_cols_idx = [col.id for col in cols if col.active]
                other_rows_idx = [row.id for row in rows if row.active]
                # print("deactivating other cols")
                # print(other_cols_idx)
                if other_cols_idx:
                    # emit a Deactivate to shut off those beams
                    program.append_inst(
                        Deactivate(
                            s=s,
                            col_objs=cols,
                            row_objs=rows,
                            qubit_objs=qubits,
                            col_idx=other_cols_idx,
                            col_xs=[cols[i].x for i in other_cols_idx],
                            row_idx=[],
                            row_ys=[],
                            dropoff_qs=[],
                            prefix=f"CleanupCols_{other_cols_idx}"
                        )
                    )
                    if other_rows_idx:
                        # emit a Deactivate to shut off those beams
                        program.append_inst(
                            Deactivate(
                                ...
                                col_idx=[],
                                col_xs=[],
                                row_idx=other_rows_idx,
                                row_ys=[rows[i].y for i in other_rows_idx],
                                dropoff_qs=[],
                                prefix=f"CleanupRxows_{other_rows_idx}"
                            )
                        )
                print("Swap neighbor appended!")
                program.append_inst(SwapNeighbor(
                    s,
                    col_objs=cols,
                    row_objs=rows,
                    qubit_objs=qubits,
                    ...
                    n=right,
                    col_idx=[c0, c1],
                    col_begin=[cols[c0].x, cols[c1].x],
                    col_end=[qubits[left].x, qubits[right].x],
                    row_idx=[qubits[left].r, qubits[right].r],
                    row_begin=[rows[r0].y, rows[r1].y],
                    row_end=[qubits[left].y, qubits[right].y]
                ))
                # program.append_inst(
                # Deactivate(
                #         s,                  # same stage
                ...
                #         row_objs=rows, 
                #         qubit_objs=qubits,
                #         col_idx   = [],     # no columns — we only want to kill the row beam
                #         col_xs    = [],
                #         row_idx   = [0],    # shut off row 0
                #         row_ys    = [rows[0].y],
                #         dropoff_qs= [],     # not dropping any atoms here
                #         prefix    = f"CleanupRow0"
                #     )
                # )
                # # fire off a SwapPair to cross‐pass those two traps
                # swap_obj.add_swap_pair(cols, rows, qubits, left, right)
    ...
    def builder_move(
        ...
        for row_id in range(self.r_high):
            if rows[row_id].active:
                ...
                # after computing col_idx, col_begin, col_end:

        # print(f"Stage {s} BigMove debugging:")
        # print("  active cols:", col_idx)
    ...
    def builder_reload(
        ...
        for q_id in range(len(qubits)):
            ...
            if (layer['qubits'][q_id]['a'] == 1):
                ...
                # print("this is row id:" + str(row_id))
    ...
    # def builder_swap_neighbors(self,
    #                         s: int,
    #                         cols: Sequence[Col],
    #                         rows: Sequence[Row],
    #                         qubits: Sequence[Qubit],
    #                         program: ComboInst):
    #     # 1) collect exactly as builder_move does
    #     col_idx, col_begin, col_end = [], [], []
    #     for cid in range(self.c_high):
    #         if cols[cid].active:
    #             print("this is cid")
    #             print(cid)
    #             col_idx.append(cid)
    #             col_begin.append(cols[cid].x)
    #             x_end = self.layers[s]['col'][cid]['x_end']
    #             off  = self.layers[s]['col'][cid]['offset_end']
    #             col_end.append(1 + x_end*X_SITE_SEP + AOD_SEP*off)

    #     row_idx, row_begin, row_end = [], [], []
    #     for rid in range(self.r_high):
    #         if rows[rid].active:
    #             row_idx.append(rid)
    #             row_begin.append(rows[rid].y)
    #             y_end = self.layers[s]['row'][rid]['y_end']
    #             off   = self.layers[s]['row'][rid]['offset_end']
    #             row_end.append(y_end*Y_SITE_SEP + AOD_SEP*(1+off))

    #     gate_qubits = []
    #     for i in range(len(qubits)):
    #         if qubits[i].array == 'AOD':
    #             gate_qubits.append(i)

    #     # 2) grab the two qubits to swap
    #     # print(cols[0])
    #     gate = self.layers[s]['gates'][0]
    #     # q0, q1 = gate['q0'], gate['q1']
    #     # q0, q1 = gate_qubits[0], gate_qubits[1]

    #     # 3) emit one high‐level SwapNeighbor that carries all that context
    #     program.append_inst(
    #         SwapNeighbor(s,
    #                     cols, rows, qubits,
    #                     gate_qubits[0], gate_qubits[1],
    #                     col_idx, col_begin, col_end,
    #                     row_idx, row_begin, row_end)
    #     )
