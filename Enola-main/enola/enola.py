...
from enola.placer.sabre_layout import place_qubit_sabre_layout, place_qubit_sabre_layout_with_backward_pass
from enola.placer.placer_hybrid import place_qubit_hybrid
    ...
    def solve(self, save_file: bool = True):
        ...
        if self.given_initial_mapping is not None:
            ...
            if self.trivial_layout:
                ...
                new_initial_mapping = place_qubit_sabre_layout((self.n_x, self.n_y), self.n_q, list_gates, self.l2)
                # new_initial_mapping = place_qubit_sabre_layout_with_backward_pass((self.n_x, self.n_y), self.n_q, list_gates, self.l2)
                # qubit_mapping = place_qubit_sabre_layout((self.n_x, self.n_y), self.n_q, list_gates, self.l2) ## forward pass only
                # qubit_mapping = place_qubit_sabre_layout_with_backward_pass((self.n_x, self.n_y), self.n_q, list_gates, self.l2)
                qubit_mapping = place_qubit_hybrid((self.n_x, self.n_y), self.n_q, list_gates, initial_mapping=new_initial_mapping, l2=self.l2)
        ...
        # print(list_gates)
        # qubit_mapping = place_qubit_sabre_layout((self.n_x, self.n_y), self.n_q, list_gates, self.l2)
        # qubit_mapping = place_qubit_sabre_layout_with_backward_pass((self.n_x, self.n_y), self.n_q, list_gates, self.l2)
        if self.to_verify:
        ...
        # program_list, time_mis, time_codeGen, time_placement = route_qubit((self.n_x, self.n_y), self.n_q, list_gates, qubit_mapping, self.routing_strategy, self.reverse_to_initial, self.l2, self.use_window)
        # qubit movement between layers
        # program_list, time_mis, time_codeGen, time_placement = custom_routing_strategy(self.n_x, self.n_y, self.n_q, list_gates, qubit_mapping, self.routing_strategy, self.reverse_to_initial, self.l2, self.use_window)
